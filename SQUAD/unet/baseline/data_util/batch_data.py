...
import numpy as np
from .glove_embedding import PAD_ID, UNK_ID


class Batch():
    """A class to hold the information needed for a training batch"""

    def __init__(self, context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, impossible, uuids=None):
        """
        Inputs:
...
def refill_batches(batches, word2id, context_file, qn_file, ans_file, impossible_file, batch_size, context_len, question_len, discard_long):
    """
    ...
    context_line, qn_line, ans_line, impossible_line = context_file.readline(), qn_file.readline(), ans_file.readline(), impossible_file.readline()# read the next line from each

    while context_line and qn_line and ans_line and impossible_line: # while you haven't reached the end

        # Convert tokens to word ids
        # context_line = ''.join("NoAnswer ",context_line)
        ...
        impossible = float(impossible_line)
        # read the next line from each file
        context_line, qn_line, ans_line, impossible_line = context_file.readline(), qn_file.readline(), ans_file.readline(), impossible_file.readline()
        ...
        # add to examples
        examples.append((context_ids, context_tokens, qn_ids, qn_tokens, ans_span, ans_tokens, impossible))
    ...
    for batch_start in range(0, len(examples), batch_size):
        ...
        context_ids_batch, context_tokens_batch, qn_ids_batch, qn_tokens_batch, ans_span_batch, ans_tokens_batch, impossible_batch = zip(*examples[batch_start:batch_start+batch_size])

        batches.append((context_ids_batch, context_tokens_batch, qn_ids_batch, qn_tokens_batch, ans_span_batch, ans_tokens_batch, impossible_batch))
...
def get_batch_generator(word2id, context_path, qn_path, ans_path, impossible_path, batch_size, context_len, question_len, discard_long):
    """
    ...
    Inputs:
      ...
      discard_long: If True, discard any examples that are longer than context_len or question_len.
        If False, truncate those exmaples instead.
    """
    context_file, qn_file, ans_file, impossible_file = open(context_path), open(qn_path), open(ans_path), open(impossible_path)
    batches = []

    while True:
        if len(batches) == 0: # add more batches
            refill_batches(batches, word2id, context_file, qn_file, ans_file, impossible_file, batch_size, context_len, question_len, discard_long)
        if len(batches) == 0:
            break

        # Get next batch. These are all lists length batch_size
        (context_ids, context_tokens, qn_ids, qn_tokens, ans_span, ans_tokens, impossible) = batches.pop(0)
        ...
        # Make impossible into anp array
        impossible = np.array(impossible) # shape (batch_size, )

        # Make into a Batch object
        batch = Batch(context_ids, context_mask, context_tokens, qn_ids, qn_mask, qn_tokens, ans_span, ans_tokens, impossible)
