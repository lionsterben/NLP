...
def get_glove(glove_path, glove_dim):
    ...
    vocab_size = int(4e5+1) # this is the vocab size of the corpus we've downloaded
    ...
    if random_init:
        emb_matrix[:len(_START_VOCAB)-1, :] = np.random.randn(len(_START_VOCAB)-1, glove_dim)
    emb_matrix[NA_ID, :] = [np.random.normal(scale=0.01) for _ in range(vec_size)]
